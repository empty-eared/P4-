...
def writeTunnelRules(p4info_helper, ingress_sw, egress_sw, tunnel_id,
                     dst_eth_addr, dst_ip_addr,SWITCH_TO_SWITCH_PORT):
    """
...
def readTableRules(p4info_helper, sw):
    ...
    for response in sw.ReadTableEntries():
        for entity in response.entities:
            ...
            action = p4info_helper.get_actions_name(entry.action.action.action_id)
            print(entry)
            print('-----')
...
def main(p4info_file_path, bmv2_file_path):
    ...
    try:
        ...
        writeTunnelRules(p4info_helper, ingress_sw=s1, egress_sw=s2, tunnel_id=100,
                         dst_eth_addr="08:00:00:00:02:22", dst_ip_addr="10.0.2.2",SWITCH_TO_SWITCH_PORT=2)

        # Write the rules that tunnel traffic from h2 to h1
        writeTunnelRules(p4info_helper, ingress_sw=s2, egress_sw=s1, tunnel_id=200,
                         dst_eth_addr="08:00:00:00:01:11", dst_ip_addr="10.0.1.1",SWITCH_TO_SWITCH_PORT=2)

        writeTunnelRules(p4info_helper, ingress_sw=s1, egress_sw=s3, tunnel_id=300,
                         dst_eth_addr="08:00:00:00:03:33", dst_ip_addr="10.0.3.3",SWITCH_TO_SWITCH_PORT=4)

        writeTunnelRules(p4info_helper, ingress_sw=s2, egress_sw=s3, tunnel_id=400,
                         dst_eth_addr="08:00:00:00:03:33", dst_ip_addr="10.0.3.3",SWITCH_TO_SWITCH_PORT=3)

        writeTunnelRules(p4info_helper, ingress_sw=s3, egress_sw=s1, tunnel_id=500,
                         dst_eth_addr="08:00:00:00:01:11", dst_ip_addr="10.0.1.1",SWITCH_TO_SWITCH_PORT=4)

        writeTunnelRules(p4info_helper, ingress_sw=s3, egress_sw=s2, tunnel_id=600,
                         dst_eth_addr="08:00:00:00:02:22", dst_ip_addr="10.0.2.2",SWITCH_TO_SWITCH_PORT=3)
        ...
        while True:
            ...
            printCounter(p4info_helper, s3, "MyIngress.ingressTunnelCounter", 500)
            printCounter(p4info_helper, s1, "MyIngress.egressTunnelCounter", 500)
